...
namespace TiledMapParser {
	...
	public class TiledLoader {
        ...
        #region me playing around
        //void LoadTileLayer(int index) {
        //	if (map.Layers.Length<=index) return;

        //	var layerType = map.Layers[index].claz;
        ...
        //					//	new object[] { Path.Combine(_foldername, tileSet.Image.FileName), tileSet.Columns, tileSet.Rows, obj });
        //					// Necessary when using separate assemblies (engine=DLL):
        //					Tile = (AnimationSprite)_callingAssembly.CreateInstance(layerType, false, BindingFlags.Default, null,
        //						new object[] { Path.Combine(_foldername, tileSet.Image.FileName), tileSet.Columns, tileSet.Rows, -1,false,addColliders }, null, null);
        //					if (Tile == null) throw new Exception("Class with name " + layerType + " not found in the calling assembly. Check namespace?");
        //				}
        //				catch (Exception error)
        //				{
        //					Console.WriteLine("Couldn't automatically create an AnimationSprite Tile with  type {0}.\n Error: {1}", layerType, error.Message);
        //					// Check the console for more information!
        //					throw error;
        //				}
